...
rs.multiFit <- function(yhat, ymat, xmat=NULL,
                        ...
                        method, dist1=dist1, weights=weights)

{
  ...
  for (kk in 1:ny)
  {
    ...
    #xiaowen added line 59-63, and change xmat=yhat[,-kk] to xmat=xmat in line 64
    xmat=yhat[,-kk]
    if(ny==2){
...
predict.rs.multiFit <- function(object, newdata, ...) {
  ...
  if (sum(bindex) != 0) {
    ...
    {
      hats <- rep(0, nrow(matrix(newdata[,bindex])))### Dr.xing revised the code at March 24, 2023
    }
