...
The MTPS package can fit models and make predictions when the
outcome is survival. We simulated a survival data to illustrate the usage. 
We used \code{simcor()} and \code{sim.censor()} to simulate survival outcomes.
...
In particular, we use the parametric AFT model(surv) as the first step method and lm1 as the second step method. 
