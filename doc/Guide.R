...
#    yy1 <- x1+x2+e
#    t1 <- exp(yy1)
#  # generate yy2 with a defined correlation coefficient with yy1
#    yy2 <- simcor(yy1, correlation = 0.1)
#    t2 <- exp(yy2)
#  # simulate survival data of yy (survival time and survival status)
#    censordat1 <- sim.censor(t1,censor.rate=0.1,censor = TRUE,task = 1)
#    censordat2 <- sim.censor(t2,censor.rate=0.1,censor = TRUE,task = 2)
#  # prepare training and test set
#    id.train <- sample(1:100, 80)
#    xmat <- data.frame(cbind(x1, x2))[id.train,]
#    xmat.test <- data.frame(cbind(x1, x2))[-id.train,]
#    ymat <- cbind(list(Surv(censordat1$ageph01[id.train],censordat1$statusph01[id.train])),list(Surv(censordat2$ageph02[id.train],censordat2$statusph02[id.train])))

## ----fit, echo = T, results = 'hide', warning=FALSE, message=FALSE, eval=FALSE----
#  # fit Residual Stacking Model for Survival Data
#   fit <- MTPS(xmat, ymat, family = 'survival', cv=F, residual = T, nfold=5, method.step1 = surv, method.step2 = lm1, dist1 = "lognormal")
